...
A typical usage is:
 ...
 """,
    ...
    parser.add_argument(
        "--model_name", type=str, required=True, help="float/float16 model to load, such as [mosaicml/mpt-7b]"
    )
    parser.add_argument(
        "--packed_model", type=str, required=True, help="float/float16 model to load, such as [mosaicml/mpt-7b]"
    )
...
def main():
    ...
    model = VQAutoModelQuantization.from_pretrained_2(model, args.packed_model, device_map="auto", **hf_args)
    tokenizer = transformers.AutoTokenizer.from_pretrained(args.tokenizer or args.packed_model, **hf_args)
    
    dataset = "wikitext2"
    model.seqlen=2048
    dataloader = torch.load("outputs/.cache/"+args.model_name+dataset+"dataloader.pth")
    testloader = torch.load("outputs/.cache/"+args.model_name+dataset+"testloader.pth")
