...
class VQuantLinear(nn.Module):
    ...
    def __init__(
        ...
        num_centroids,num_res_centroids = self.adjust_num_centroids([out_features,in_features], \
                                                                    num_centroids,num_res_centroids, \
                                                                    group_size,vector_lens[1])
    ...
    def adjust_num_centroids(self,shape,num_centroids,num_res_centroids,
                             group_size,vector_len):
        for i,num in enumerate(num_centroids):
            if (shape[0]*group_size)//vector_len < num:
                num_centroids[i] = num_centroids[i]//256
                
        for i,num in enumerate(num_res_centroids):
            if (shape[0]*group_size)//vector_len < num:
                num_res_centroids[i] = num_res_centroids[i]//256
        return num_centroids,num_res_centroids
    ...
    def init_parameters(
        ...
        _centroids = self.pad_and_stack_centroids_index(_centroids)
        ...
        _indices = self.pad_and_stack_centroids_index(_indices)
        ...
        if self.enable_residual:
            ...
            # (num_codebooks, num_centroids, vector_len)
            _res_centroids = self.pad_and_stack_centroids_index(_res_centroids)
            ...
            if len(res_centroids.keys()) > len(res_indices.keys()):
                for _ in range(len(res_centroids.keys()) - len(res_indices.keys())):
                    _res_indices.append(None) 
            _res_indices = self.pad_and_stack_centroids_index(_res_indices)
    ...
    def pad_and_stack_centroids_index(self, data):
        shape = data[0].shape
        # for d in data:
        #     if shape[0]<d.shape[0]:
        #         shape = d.shape
        
        for i in range(1,len(data)):
            if data[i] is None:
                data[i] = torch.zeros_like(data[0])
            if data[i].shape[0] < shape[0]:
                data[i] = F.pad(data[i],(0,0,0,shape[0] - data[i].shape[0]))
