...
def get_llama(model_name, seqlen=None):
    ...
    # model = LlamaForCausalLM.from_pretrained(model_name, attn_implementation="eager", torch_dtype=torch.bfloat16)
    import models
    model = models.LlamaForCausalLM.from_pretrained(model_name, attn_implementation="eager", torch_dtype=torch.bfloat16)
...
def quant_llama(model, args, quant_args, dev='cuda'):
    ...
    if args.vq_type == 'vptq':
        print('Starting VPTQ...')
        from vptq.quantize_executer import quantize_executer
    elif args.vq_type == 'rpvq':
        print('Starting VPTQ...')
        from vptq.quantize_executer import quantize_executer
    elif args.vq_type == 'rpvq_v1':
        print('Starting RPVQV1...')
        from rpvq_v1.quantize_executer import quantize_executer
    elif args.vq_type == 'rpvq_v2':
        print('Starting RPVQV2...')
        from rpvq_v2.quantize_executer import quantize_executer
    elif args.vq_type == 'rpvq_v3':
        print('Starting RPVQV3...')
        from rpvq_v3.quantize_executer import quantize_executer
    else:
        raise ValueError(f'vq_type {args.vq_type} not supported')
    ...
    if args.num_gpus == 1:
        layer_state_dicts,quantizers,layer_qlinear_args = quantize_executer(0, tasks[0], args, quant_args, None, None)
    else:
        for gpu_idx in range(args.num_gpus):
            # we have to set CUDA_VISIBLE_DEVICES here
            # cuml only supports to run on GPU:0
            os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu_ids[gpu_idx])
            p = mp.Process(
                target=quantize_executer,
                args=(
                    args.gpu_ids[gpu_idx],
                    tasks[gpu_idx],
                    args,
                    quant_args,
                    input_queues,
    ...
    if args.num_gpus > 1:
        ...
        if args.save_qlinear:
            ...
            while not output_queues.empty():
                (gpu_id, layer_idx, _layer_state_dict, _quantizers, _layer_qlinear_args) = output_queues.get()
                ...
                if layer_idx not in quantizers:
                    quantizers[layer_idx] = _quantizers
    ...
    if 'vptq' in args.vq_type:
        qmodel = get_quantized_llama(model_name, args.seq_len, layer_state_dicts, layer_qlinear_args)
    elif 'rpvq' in args.vq_type:
        qmodel = get_quantized_llama_rpvq(model_name, args.seq_len, layer_state_dicts, layer_qlinear_args)
    else:
        qmodel=model
    model = qmodel
...
def get_quantized_llama_rpvq(model_name, seqlen, layer_state_dicts, layer_qlinear_args):
