...
class NPVectorQuantizer:
    ...
    def symmetric_quantize(self, tensor, bit_width=8):
        qmin = -(2 ** (bit_width - 1))
        ...
        if not hasattr(self,"scale"):
            if max_val == 0:
                ...
                self.scale = max_val / qmax
        quantized_tensor = np.clip(np.round(tensor / self.scale), qmin, qmax).astype(np.int8)
        dequantized_tensor = quantized_tensor.astype(np.float32) * self.scale
    ...
    def adjust_num_centroids(self,shape):
        for i,num in enumerate(self.num_centroids):
            if (shape[0]*self.group_size)//self.vector_len < num:
                self.num_centroids[i] = num
                
        for i,num in enumerate(self.num_res_centroids):
            if (shape[0]*self.group_size)//self.vector_len < num:
                self.num_res_centroids[i] = num
    ...
    def init_centroids_indices(self, data, weights=None):
        ...
        self.adjust_num_centroids(data.shape)
        quantized_data = []
        ...
        for idx, (i, j) in enumerate(self.get_index_list(data)):
            ...
            if num_centroids == -1:  # Do not quantize, keep original data
                ...
                try:
                    ...
                    _kmeans.fit(sub_vectors, sample_weight=vector_weights.cpu())
                
                if self.codebook_bitwidth != -1:
                    self.symmetric_quantize(_kmeans.cluster_centers_,bit_width=self.codebook_bitwidth)
    ...
    def init_res_centroids_indices(self, data, weights=None):
        ...
        for idx, (i, j) in enumerate(self.get_index_list(data)):
            vector_len = self.outlier_vector_len if idx == 0 else self.vector_len
            if len(self.num_res_centroids)>2:
                num_centroids = self.num_res_centroids[idx]
            else:
            ...
            if num_centroids == -1:
                ...
                if self.codebook_bitwidth and _kmeans.cluster_centers_ is not None:
                    self.symmetric_quantize(_kmeans.cluster_centers_,bit_width=self.codebook_bitwidth)
